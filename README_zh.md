...
### OCR 图像识别
- **图片文字识别**：支持单张图片、多张图片和 PDF 文件的文字提取
- **多种处理模式**：批处理模式（每张图片生成独立记录）和顺序处理模式（合并为单条记录）
- **PDF 支持**：自动将 PDF 页面转换为图片后进行顺序 OCR 识别
...
│   │   │   ├── OcrView.vue           # OCR 图像识别（拖拽上传、批处理/顺序处理）
│   │   │   ├── SettingsView.vue      # 完整设置（通用、模型、AI、ASR、OCR 选项卡）
...
完整更新日志请查看 [reference/Changelog.md](reference/Changelog.md)。

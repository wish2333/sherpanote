...
│   │   │   ├── useAiPresets.ts       # AI API 预设管理（从 SettingsView 提取）
│   │   │   ├── useProcessingPresets.ts # AI 处理预设模板
│   │   │   ├── useBackup.ts          # 备份导入导出逻辑
│   │   │   ├── useAudioPlayback.ts   # 音频播放控制（从 EditorView 提取）
│   │   │   └── useVersionManagement.ts # 版本历史保存/恢复
│   │   ├── stores/
...
│   ├── asr_recognizer.py      # 识别器工厂函数（在线、离线、whisper）
│   ├── file_matcher.py        # 共享模型文件匹配与分类
│   ├── llm.py                 # AI 文本处理（流式）
│   ├── config.py              # 应用配置管理
│   ├── storage.py             # SQLite 持久化 + 版本控制
│   ├── model_manager.py       # 模型下载、安装、验证（5 种源）
│   ├── model_registry.py      # 模型目录（10+ 模型）和 URL 常量
...
│   ├── adapters/              # 后端适配器（ppocr、markitdown、docling、opendata）
│   ├── outputs/               # 统一输出类型（ExtractedDocument）
│   ├── plugins/               # 插件系统
│   │   └── runners/           # 插件运行器（docling、opendataloader）
│   ├── api/                   # Bridge 暴露的 API 方法（Mixin 架构）
│   │   ├── base.py            # ApiBase - 共享状态访问
│   │   ├── asr.py             # AsrMixin - ASR、whisper.cpp、依赖管理
│   │   ├── ai.py              # AiMixin - AI 处理、预设管理
│   │   ├── storage.py         # StorageMixin - 记录 CRUD、版本、音频、导入导出
│   │   ├── models.py          # ModelsMixin - ASR 模型管理
│   │   ├── ocr_plugin.py      # OcrPluginMixin - OCR、插件、文档提取
│   │   └── config_backup.py   # ConfigBackupMixin - 配置、备份恢复、文件选择
│   ├── whispercpp.py          # Whisper.cpp ASR 后端集成
│   ├── video_downloader.py    # 视频下载转录
│   └── io.py                  # 音频 I/O 工具
├── pywebvue/           # PyWebVue 框架核心
├── main.py             # 应用程序入口（SherpaNoteAPI 由领域 Mixin 组合）
├── dev.py              # 开发启动脚本
├── build.py            # 构建和打包脚本
...
[2026-05-08 - v2.1.1 代码质量与安全加固]

### 安全修复

- 文件/文件夹打开操作的路径遍历攻击防护（白名单校验）
- 备份导入 ZipSlip 漏洞修复（路径边界检查）
- Markdown 渲染器 XSS 防护（HTML 转义 + 链接协议白名单）

### 架构重构

- `main.py` 从 2343 行重构至约 250 行，SherpaNoteAPI 拆分为 6 个领域 Mixin 类，置于 `py/api/` 目录
- 提取 `py/file_matcher.py`（共享模型文件匹配）和 `py/asr_recognizer.py`（识别器工厂）
- 从过大的视图中提取 5 个前端 composable：`useAiPresets`、`useProcessingPresets`、`useBackup`、`useAudioPlayback`、`useVersionManagement`

### 可靠性修复

- 修复 `@expose` 装饰器、AI 处理、数据库迁移和剪贴板 API 的静默错误吞没
- 修复组件卸载时事件监听器和定时器未清理导致的内存泄漏
- 为模型下载代理配置和 OCR 引擎延迟初始化添加线程安全保护

### 代码质量

- 6 个文件中 10+ 处魔法数字替换为命名常量
- 硬编码 URL 整合到 `model_registry.py` 共享常量
- 改进 TypeScript 类型安全，添加类型守卫，移除 `any` 类型断言
- 错误消息不再泄露系统路径

...
```
refactor(core): 架构重构与全量安全/可靠性修复

- 拆分上帝对象：将 2343 行的主模块按职责重构为 6 个独立 mixin 类
- 修复安全漏洞：阻断文件打开/备份导入的路径遍历及 Markdown 渲染的 XSS 风险
- 提升可靠性：精确捕获异常避免静默吞错，修复事件监听器及定时器导致的内存泄漏，增加环境变量线程锁
- 代码优化：提取公共模块与前端可组合函数消除冗余，清除魔法数字与硬编码 URL，完善 TS 类型与系统文档
```

### 🚀 Release Notes

```
## 2026-05-08 - 核心架构升级与安全性增强

### 🐛 修复
- 修复多项潜在安全风险：阻断了文件打开、备份导入中的恶意路径访问，以及 Markdown 渲染的跨站脚本攻击（XSS）漏洞
- 修复 AI 处理、数据库迁移等模块因静默吞没异常导致的状态无响应或操作失效问题，显著提升系统稳定性

### ⚡ 优化
- 核心架构解耦：后端主模块按职责全面拆分，大幅降低代码复杂度，提升后续功能迭代的开发效率
- 前端组件瘦身：提取并统一管理跨组件复用逻辑，消除冗余代码，提高页面渲染性能
- 资源管理优化：全局排查并修复多个页面中事件监听与定时器未及时释放导致的内存泄漏隐患
- 开发体验升级：消除代码中的魔法数字，强化前端类型安全校验，并补充了完整的开发指南与变更日志文档
```
